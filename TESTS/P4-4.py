...
# ---------- FONCTIONS POUR L'IA DE L'ORDINATEUR ----------
...
def jouer(grille, j, j_nom):
    ...
    Joue un coup du joueur j dans la colonne choisie.
    Renvoie la grille mise à jour et un booléen indiquant si le coup a été joué.
    Le joueur peut quitter en tapant 0.
    """
    while True:
        try:
            colonne = int(input(f"{j_nom}, dans quelle colonne voulez-vous jouer? (1-7 ou 0 pour quitter) : "))

            # Option pour quitter le jeu
            if colonne == 0:
                print(f"{j_nom} a choisi de quitter la partie.")
                return grille, False, True  # Le troisième paramètre indique qu'il faut quitter

            colonne -= 1  # Ajustement pour l'indexation commençant à 0

            # Vérification que la colonne est valide
            if colonne < 0 or colonne > 6:
                print("Erreur: Veuillez entrer un nombre entre 1 et 7 (ou 0 pour quitter).")
                continue
            ...
            if possible:
                if j == 1:
                    ...
                    grille[ligne][colonne] = 'V'
                return grille, True, False  # Le coup a été joué, ne pas quitter
            else:
                print("Cette colonne est pleine. Veuillez choisir une autre colonne.")
        except ValueError:
            print("Erreur: Veuillez entrer un nombre valide.")
...
# ---------- FONCTIONS POUR L'ALGORITHME MINIMAX ----------
...
def evaluer_position_minimax(grille, symbole):
    ...
    centre = [grille[i][3] for i in range(6)]
    count_centre = centre.count(symbole)
    score += count_centre * 3
...
def est_terminal(grille):
    ...
    Renvoie (True, valeur) si terminal, (False, 0) sinon.
    """
    # Vérification de victoire
    if verifier_victoire(grille, 'V'):
        return True, 1000000  # Victoire de l'ordinateur

    if verifier_victoire(grille, 'B'):
        return True, -1000000  # Victoire du joueur
...
def minimax(grille, profondeur, alpha, beta, maximisant):
    ...
    Renvoie (score, colonne) où colonne est le meilleur coup à jouer.
    """
    ...
    for col in colonnes_valides:
        ...
        if possible:
            # Simulation du coup
            grille_temp = [ligne[:] for ligne in grille]  # Copie de la grille
            grille_temp[ligne][col] = symbole

            # Appel récursif pour évaluer ce coup
...
def coup_ordi_minimax(grille, difficulte):
    ...
    if difficulte == 3:
        profondeur = 5  # Difficile
    else:
        # Conserver les anciennes implémentations pour les niveaux 1 et 2
        if difficulte == 1:
            return coup_ordi_facile(grille)
        else:  # difficulte == 2
            return coup_ordi_moyen(grille)

    # Vérification rapide pour un coup gagnant immédiat
    col_gagnante = coup_gagnant(grille, 'V')
    if col_gagnante is not None:
        return col_gagnante

    # Vérification rapide pour bloquer un coup gagnant de l'adversaire
    col_bloquante = coup_gagnant(grille, 'B')
    if col_bloquante is not None:
        return col_bloquante
...
# Mise à jour de la fonction jouer_ordi pour utiliser Minimax
def jouer_ordi(grille, difficulte):
    ...
    if difficulte == 1:
        ...
        colonne = coup_ordi_minimax(grille, difficulte)
...
def puissance4_vs_ordi():
    ...
    while True:  # Boucle principale du jeu
        ...
        grille, coup_joue, quitter = jouer(grille, 1, j_1)  # Le joueur joue son coup

        if quitter:  # Si le joueur a choisi de quitter
...
def puissance4_2joueurs():
    ...
    while True:
        ...
        grille, coup_joue, quitter = jouer(grille, 1, j_1)
        ...
        grille, coup_joue, quitter = jouer(grille, 2, j_2)
